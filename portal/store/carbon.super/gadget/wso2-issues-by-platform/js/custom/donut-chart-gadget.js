...
(function () {
    ...
    var configure = function () {
        ...
        var getStateDetailsById = function (stateId) {
            ...
            $.each(WSO2_PRODUCT_ISSUES_DATA, function (index, element) {
        ...
        /*
         * Get the openissues data for each state
         * */
        var getOpenIssuesByState = function (stateId) {
            var openIssues_data = [],
                data = getStateDetailsById(stateId);
            debugger;
            $.extend(true, openIssues_data, data.component);
            return openIssues_data;
        };

        /*
         * Get the openIssues by component
         * */
        var getOpenIssuesByComponent = function (componentDetails, component) {
            var component = [];

            $.each(componentDetails, function (index, element) {
                if (element.name == component) {
                    $.extend(true, component, element.component);
                    return false;
                }
            });
        ...
        var initUI = function () {
            ...
            methodsForChangeDonut[PRODUCT_CHANNEL] = updateChartAccordingToProduct;
            ...
            gadgets.HubSettings.onConnect = function () {
                ...
                gadgets.Hub.subscribe(PRODUCT_CHANNEL, function (topic, message) {
                    callbackForChannel(PRODUCT_CHANNEL, message);
                });
        ...
        var updateChartAccordingToState = function (message) {
            debugger;
            var platformData = getOpenIssuesByState(message.data);
            update(platformData, null, null, false);
        };

        /*
         * Update the donut chart according to the incoming message from the Product channel.
         * @private
         * */
        var updateChartAccordingToProduct = function (message) {
            var stateDetails = getStateDetailsById(message.state);
            var componentData = getOpenIssuesByComponent(stateDetails.component, message.data);
            debugger;
            update(componentData, null, null, false);
        };
        ...
        var filterData = function (element, index) {
            ...
            element.value = streakerDataAdded[index].totalOpenIssues;
            totalOpenIssues += element.value;
        ...
        var updateChartDetails = function (data) {
            ...
            data.forEach(function (element, index) {
                var html = "<li>" + "<div style='background-color:" + color(element) + ";'></div><span>" + element.name + " - " + element.totalOpenIssues + "</span></li>";
         ...
         * */
        var publishPlatformData = function (platformId) {
            debugger;
            var dataBundle = {
                data: platformId,
                state: subscribeData.state
            };
            gadgets.Hub.publish(PLATFORM_CHANNEL, dataBundle);
        };
        ...
        var update = function (dataToProcess, parentName, prevParent, isBack) {
            ...
            paths.on("click", function (d) {
                ...
                if (d.data.component.length > 0) {
                    allPaths.transition().duration(tweenDuration).attrTween("d", pieTween);
                    $("#back").show();
                    update(d.data.component, d.data.name, null, false);
                }

                if (d.data.name != "Male" && d.data.name != "Female") {
                    publishPlatformData(d.data.id);
                }
            })
     ...
     **/
    initDonutChart = function (properties, response) {
        WSO2_PRODUCT_ISSUES_DATA = response.data;
        dispose();
        state = properties;
        configure();
    };
